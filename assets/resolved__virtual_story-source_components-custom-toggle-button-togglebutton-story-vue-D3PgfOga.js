const resolved__virtual_storySource_componentsCustomToggleButtonTogglebuttonStoryVue = `<script lang="ts" setup><\/script>

<template>
  <Story title="Toggle|Swap Button" :layout="{ type: 'grid', width: '300px' }">
    ...
    <Variant title="Swap Button">
      <div class="flex gap-2">
        <SwapButton :states="[{ id: 'On' }, { id: 'Off' }, { id: 'Auto' }]" />
        <SwapButton
          v-slot="state"
          :selected="{ id: 'moon' }"
          :states="[
        ...
        >
          <SwapItem v-bind="{ ...state }" style="backface-visibility: hidden" />
        </SwapButton>
      </div>
    </Variant>
  </Story>
...
`;
export {
  resolved__virtual_storySource_componentsCustomToggleButtonTogglebuttonStoryVue as default
};
